    ...
    useImperativeHandle(ref, () => ({
        drop() {
            ...
            for (let i = 0; i < rows; i++) {
                // Skew the random bounce to guide it towards the outcome
                let direction = Math.random() < 0.5 ? -0.5 : 0.5;
                const targetColumn = (outcomeBin / (multipliers.length -1)) * (i + 1);
                const currentColumn = ( (path[path.length-1].x - (50 - (i*2.8))) / 5.6 );
                ...
                currentPathX += direction * (100 / (i + 15));
            ...
            setBalls(prev => [...prev.slice(-15), { id: Date.now(), path, outcomeBin }]);
        }
    }));
...
export default function PlinkoPage() {
    ...
    useEffect(() => {
        if (isTxSuccess) {
            ...
            refetchMainBalance();
            refetchGameBalance(); // This will trigger the other useEffect to sync the local balance
            reset();
        }
    ...
    return (
        <main className="mx-auto max-w-7xl px-6 py-12">
            <DepositWithdrawModal 
                ...
                gameBalance={onChainGameBalance ?? BigInt(0)} // Always show true balance in modal
                ...
                <div className="lg:col-span-1 space-y-6 rounded-2xl border border-white/10 bg-white/[0.02] p-6 sticky top-24">
                    ...
                    <div className="p-4 rounded-lg bg-black/20 text-center">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-3">
                            ...
                            <button onClick={handleWithdraw} className="btn-ghost" disabled={isTxLoading || localGameBalance === BigInt(0)}>Withdraw All</button>
                        </div>
                    </div>
                    <div>
